...
const { data: count } = await useAsyncData(
  "branch_total",
  async () => (await useCountBranchMetricApi('total')).data.data
);
const filter = ref();
...
<template>
  <section class="bg-primary-50 py-20">
    <div class="container mx-auto px-3 flex flex-col lg:flex-row gap-14 gap-20 lg:items-center">
      <div class="flex flex-col gap-3 w-96 flex-none">
        <h3 class="text-3xl font-extrabold text-primary-500">{{ count.value }} cửa hàng</h3>
        <p class="text-xl font-light text-default-700">Cửa hàng Doodle trải dài trên khắp toàn quốc để luôn sẵn sàng hỗ trợ quý khách.</p>
        <div class="mt-5">
