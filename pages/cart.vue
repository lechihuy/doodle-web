...
<template>
  <div class="relative">
    <div v-show="cart.loading" class="text-default-500 flex items-center justify-center gap-1 min-h-[calc(100vh_-_theme(space.32))]">
      <IconLoading class="w-5 h-5" /> Đang tải
    </div>

    <div v-if="cart.count > 0 && !cart.loading" class="flex mb-16 flex-col lg:flex-row container px-3 mx-auto min-h-[calc(100vh_-_theme(space.32))] lg:divide-x divide-default-900/10">
      <div class="grow py-7 lg:pr-7">
        ...
        <div class="flex flex-col divide-y divide-default-100">
          <div v-for="item in cart.data.items" :key="item" class="flex gap-5 py-5">
            <nuxt-img :src="item.product.thumbnail.url" class="w-24 bg-default-50 h-24 rounded-lg" />
            <div class="grid grid-cols-1 content-between">
              <NuxtLink class="font-semibold text-primary-950 line-clamp-2">
                {{ item.product.name }}
              </NuxtLink>
              <div class="inline-flex">
                <AddToCartButton :product-id="item.product.id" size="mini" />
              </div>
            </div>
    ...
    <div v-if="cart.count == 0 && !cart.loading" class="flex container px-3 mx-auto justify-center min-h-[calc(100vh_-_theme(space.16))] items-center">
  ...
  <div v-if="!cart.loading && cart.count > 0" class="fixed bottom-0 w-full h-16 border-t border-default-100 bg-default-50" :style="{ zIndex: zIndex.modal }">
    <div class="flex items-center h-full justify-end gap-5 container px-3 mx-auto">
      <div class="flex items-center gap-2">
        ...
        <p class="font-bold text-danger-500">{{ currency(cart.data.total) }}</p>
      </div>
